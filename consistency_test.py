...
def check_alignment_with_memory(
    ...
    aligned = (
        ...
        or (expected == "maintain_relationship_proactively" and ("maintain" in resp_lower or "proactive" in resp_lower or "reach out" in resp_lower or "contact" in resp_lower or "active" in resp_lower or "relationship" in resp_lower or "silence" in resp_lower or "主動" in resp_lower or "維護" in resp_lower or "聯繫" in resp_lower))
        or (expected == "build_framework_first" and ("framework" in resp_lower or "skeleton" in resp_lower or "structure" in resp_lower or "overview" in resp_lower or "map" in resp_lower or "toc" in resp_lower or "outline" in resp_lower or "骨架" in resp_lower or "框架" in resp_lower or "全局" in resp_lower or "先" in resp_lower))
        or (expected == "track_behavior_not_speech" and ("behavior" in resp_lower or "action" in resp_lower or "track" in resp_lower or "observe" in resp_lower or "consistent" in resp_lower or "commitment" in resp_lower or "fulfillment" in resp_lower or "行為" in resp_lower or "言語" in resp_lower or "一致性" in resp_lower or "承諾" in resp_lower or "兌現" in resp_lower))
    )
