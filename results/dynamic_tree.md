...
- 1.1 Trading system → live profit
  ...
  - **mainnet_runner.py kill-rail bug fixed ✓** (cycle 54) — MDD>10% kill was defined but never checked; `_compute_stats` now filters to EXECUTED-only for PnL/WR/PF; `_check_kill` uses `trades` count not `ticks` count; `mdd_pct` now computed + displayed
...
### 6. 存活冗餘（anti-fragile）
- 6.1 冷啟動 recovery：templates/dna_core.md **267 MDs** ✓ (cycle 53: 201709+201708 → MD-265~267; distill chain: 202604→201707, 53 months processed)
...
- 2026-04-08 UTC: cycle 54 — 2 bug fixes: mainnet_runner.py MDD kill-rail now enforced + stats use EXECUTED-only; tmp_distill.py handles pre-2018 s/t schema + CLI args
