...
- 2.3 Validation：OOS 5/5 self-scored ✓, **318-MD consistency test 16/16 ALIGNED ✅** (cycle 91; prev: 14/14 cycle 90 at 318 MDs)
  - 0 MISALIGNED — 2 new scenarios added this cycle:
    - `generic_debt_vs_invest`: MD-316 (比較利率%不比較金額) → COMPARE_RATES_NOT_AMOUNTS ✓
    - `generic_dca_execution`: MD-318 (DCA=分析通過後預設執行) → DCA_IS_DEFAULT_EXECUTION ✓
  - `capital_allocation` domain added to `_domain_decision` + `DOMAIN_PRINCIPLE_AFFINITY` in organism_interact.py
  - baseline saved: results/consistency_baseline.json (**16 scenarios**, 16 ALIGNED; 318 MDs validated)
