...
- 2.3 Validation：OOS 5/5 self-scored ✓, **303-MD consistency test 13/14 ALIGNED ✓** (cycle 81; prev: 285-MD 12/12 cycles 57→75)
  - 1 MISALIGNED: generic_strategy_failure (expected=DEFINE_KILL_CONDITIONS_FIRST) — needs boot_tests.json fix or dna_core clarification
  - consistency_test.py: --use-memory + --auto-suggest now combined — memory context flows into suggestions ✓
  - baseline saved: results/consistency_baseline.json (**14 scenarios**, 13 ALIGNED; +2 new domains: information_asymmetry + negotiation cycle 75)
